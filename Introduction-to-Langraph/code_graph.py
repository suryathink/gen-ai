...
def classify_message(state:State):
    ...
    SYSTEM_PROMPT = """
    You are an AI assistant. Your job is to detect if the user's
    query is related to coding question or not.
    ...
    response = client.beta.chat.completions.parse(
        model="gpt-4.1-nano",
        response_format=ClassifyMessageResponse, # adding this to get response in structured format
        messages=[
    ...
    print("is_coding_question",is_coding_question)
...
def coding_validate_query(state:State):
     ...
     SYSTEM_PROMPT = f"""
       You are an expert in calculating accuracy of the code
     ...
     response = client.beta.chat.completions.parse(
        model="gpt-4.1",
        response_format=CodeAccuracyResponse, # adding this to get response in structured format
        messages=[
            {"role" : "system", "content":SYSTEM_PROMPT},
            {"role" : "user", "content":query},
        ]
...
def main():
    ...
    print("main method response --> ", response)
